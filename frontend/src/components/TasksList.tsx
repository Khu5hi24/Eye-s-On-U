        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {statusOptions.map((opt) => (
              <option key={opt.value} value={opt.value} className='rounded-2xl'>{opt.label}</option>
            ))}
          </select>
          ...
          {(filters.status || filters.priority || filters.searchQuery || filters.sortBy) && (
            <Button variant="ghost" size="sm" onClick={resetFilters} className="h-10 rounded-2xl text-xs font-bold uppercase tracking-wider text-rose-500 hover:bg-rose-500">
              Reset Filters
            </Button>
          )}
        </div>
